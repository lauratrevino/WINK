...
def init_db():
    ...
    try:
        ...
        cur.execute("ALTER TABLE students ADD COLUMN IF NOT EXISTS first_generation BOOLEAN DEFAULT FALSE")
...
def register():
    ...
    try:
        if request.method == "POST":
            ...
            cur.execute("""INSERT INTO students(email,password_hash,first_name,last_name,classification,major,university,first_generation)
                           VALUES(%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id""",
                        (email, generate_password_hash(pw), fn, ln, cl, major, university, first_generation))
    ...
    try:
        if request.method == "POST":
            ...
            if s and check_password_hash(s["password_hash"], pw):
                ...
                # Admin goes straight to the Admin page
...
def chat():
    try:
        ...
        instructions = (
            ...
            f"5. SOURCE CITATIONS: Always cite the source of factual information, but NEVER narrate the retrieval process or say what was not found. "
            f"Do not say phrases such as 'I could not find this in your documents', 'your documents do not contain', 'I searched the web', or 'this came from a web search'. Just give the answer confidently and cite it. "
            f"For student-uploaded material, cite the actual document name, for example [Source: UNIV 1301 Syllabus, Fall 2026]. For extracted deadlines, cite the originating document name when available. "
            f"For GENERAL REFERENCE DOCUMENTS, cite the actual reference document name without describing it as a global/admin document. "
            f"For online information, cite the specific authoritative organization/page and include its URL when available. Prefer official university sources for university facts. "
            f"If multiple sources support the answer, cite the relevant sources. Never fabricate a citation, document title, page, URL, or source. "
            f"6. ACCURACY AND CONFIDENCE: Read the relevant student documents carefully enough to resolve section-specific facts before answering. "
            f"Use the student's known course/section context and exact document details to choose the correct person, deadline, policy, location, or requirement. "
            f"Do not present multiple possibilities when the available evidence identifies one answer. If sources genuinely conflict or remain ambiguous after checking them, say exactly what conflicts and ask only for the missing fact needed to resolve it. "
            f"7. UNIVERSITY KNOWLEDGE: After student documents and GENERAL REFERENCE DOCUMENTS, use authoritative university-specific online information and repositories for {university_display} before broader external/general web sources. "
            ...
            f"CRITICAL THINKING & GROWTH MINDSET: First classify the student's question by what it requires. "
            f"For simple factual or navigational questions (where, when, who, contact information, straightforward deadlines), answer directly and completely; do not turn them into a thinking exercise. "
            f"For questions involving decisions, interpretation, problem-solving, planning, judgment, reflection, comparison, evaluation, or learning a concept, use the student's uploaded documents and relevant university information to guide critical thinking. "
            f"Give any essential factual information the student needs, then ask ONE focused question at a time that helps the student analyze evidence, identify assumptions, consider alternatives or consequences, make connections, explain reasoning, or decide what information is still needed. "
            f"Make the question specific to the student's actual course, assignment, policy, schedule, resources, or situation whenever the available documents support that context. "
            f"Do not interrogate the student, withhold necessary information, or stack several questions at once. As the conversation continues, build on the student's previous answer so the questioning becomes progressively more thoughtful. "
            f"The goal is to model a repeatable thinking process the student can eventually use independently. Keep it natural, supportive, and concise."
        )
    ...
    try:
        ...
        cur.execute("""
            SELECT
                ...
                s.classification, s.major, COALESCE(s.first_generation, FALSE) as first_generation,
