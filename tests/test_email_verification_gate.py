"""
Real tests for the email-verification gate — deliberately does NOT use
the shared register() helpers from other test files, since those mark the
student verified immediately (realistic for testing everything else).
This file tests the gate itself, so it registers without verifying.
"""
import io
...
def register_unverified(client, email="student@utep.edu"):
    """Registers WITHOUT marking the student verified — the real, default
    state for a brand-new account before it clicks the verification
    email."""
...
class TestEmailVerificationGate:
    ...
    def test_unverified_student_can_still_view_dashboard(self, client):
        """The gate is deliberately scoped to cost-generating actions only
        — an unverified student shouldn't be completely locked out of
        their own account, just unable to use chat/uploads yet."""
        register_unverified(client)
        resp = client.get("/dashboard")
        assert resp.status_code == 200

    def test_verified_student_can_chat_and_upload(self, client, app, monkeypatch):
        """Real end-to-end: register, verify via the real token from the
        real events log, then confirm chat/upload actually work."""
        import wink.blueprints.chat as chat_bp
...
class TestPasswordResetTokenHashing:
    def test_full_reset_flow_works_with_hashed_token_storage(self, client, app, monkeypatch):
        """Real end-to-end: register, request a reset, capture the RAW
        token the way a real email would carry it (via
        DEBUG_SHOW_RESET_LINKS — never by reading the DB, since the DB now
        only ever holds the hash), reset the password with it, and confirm
        login actually works with the new password."""
        import re
        ...
        # Confirm the DB genuinely does NOT hold the raw token anywhere.
        with app.app_context():
            conn = get_db(); cur = conn.cursor()
