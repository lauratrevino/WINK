...
def delete_account():
    ...
    account_deleted_at, which also excludes them from deadline reminders
    and other ongoing automated processes, see
    ...
    These two things happen in one transaction on purpose: as two
    separate commits, a failure on the second one would leave the
    account disabled — the student logged out, unable to log back in —
    while their real name and email were still sitting in the database,
    un-anonymized. Either both changes land together, or neither does;
    there's no partial state where deletion "succeeded" from the
    account's perspective but the anonymization it promised silently
    didn't happen.
