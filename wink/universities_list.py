"""The full list of universities offered in the registration dropdown
(templates/register.html) — extracted once from that same dropdown so
there's exactly one source of truth for "which universities are valid,"
not two lists that could drift apart. Kept in its own file since it's
too large (2,338 entries) to sit comfortably inline in config.py.

Used server-side to validate that a submitted `university` value is
actually one of the real options — without this, the dropdown was only
a client-side convenience; a raw POST could set `university` to any
string at all, including another real institution's exact name, which
would then surface that institution's global reference documents (see
students/documents.py: global docs are filtered by
`lower(university)=lower(%s)`)."""
