    ...
    async function viewDemoConversation(sid) {
      ...
      if (!overlay) {
        ...
        overlay.innerHTML = `<div style="background:#fff;border-radius:12px;max-width:640px;width:90%;max-height:80vh;display:flex;flex-direction:column;">
          <div style="padding:16px 20px;border-bottom:1px solid #eef0f6;display:flex;justify-content:space-between;align-items:center;">
            <h3 style="margin:0;font-size:16px;color:#002855;">Demo conversation</h3>
      ...
      try {
        ...
        if (!convos.length) {
          body.innerHTML = '<p style="color:#6b7a99;">No conversation recorded for this demo.</p>';
          return;
        }
        body.innerHTML = convos.map(c => `
          <div style="margin-bottom:16px;">
            <div style="font-size:11px;color:#6b7a99;margin-bottom:4px;">${escapeHtml(c.ts)}</div>
